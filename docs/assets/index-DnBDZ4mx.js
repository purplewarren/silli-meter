...
If you have specific concerns about your child's eating, consider consulting with a pediatrician or registered dietitian.`}displayAnswer(e){const t=this.container.querySelector("#answer-section"),s=this.container.querySelector("#answer-content");if(t&&s){t.style.display="block",s.innerHTML=`
    ...
    `}function i(){const l=e.map(h=>h.trigger).filter(h=>h!==void 0);if(l.length===0)return null;const u=l.reduce((h,m)=>(h[m]=(h[m]||0)+1,h),{});return Object.entries(u).reduce((h,m)=>h[1]>m[1]?h:m)[0]}function o(){const l=r.querySelector(".history-sessions");if(l){if(e.length===0){l.innerHTML=`
      ...
      `;return}l.innerHTML=e.map(u=>{const h=new Date(u.ts),m=w(h),y=u.intensity_1_10||0,S=u.escalation_index||0,C=y<=3?"#4CAF50":y<=6?"#FF9800":"#F44336",g=S<=.3?"#4CAF50":S<=.7?"#FF9800":"#F44336";return`
    ...
    `;const T=document.createElement("div");T.className="history-content",T.style.cssText=`
      display: none;
    `;const B=k("label",{text:d.forms.trigger,id:"tantrum_trigger_label"});B.htmlFor="tantrum_trigger",e=k("select",{id:"tantrum_trigger"});const I=document.createElement("option");I.text=d.placeholders.selectTrigger,I.value="",e.appendChild(I),he.forEach(M=>{const E=document.createElement("option");E.text=M.charAt(0).toUpperCase()+M.slice(1),E.value=M,e.appendChild(E)}),v.appendChild(B),v.appendChild(e);const _=document.createElement("div");_.style.cssText=`
      margin: 16px 0;
      padding: 16px;
      background: #f9f9f9;
      border-radius: 8px;
    `,w=oe(),w.mount(_),v.appendChild(_);const L=k("label",{text:"Media Clip (Optional)",id:"tantrum_media_label"});L.htmlFor="tantrum_media",a=k("input",{id:"tantrum_media"}),a.type="file",a.accept="audio/*,video/*",a.capture="environment",a.style.cssText=`
      margin: 8px 0;
      padding: 8px;
    ...
    `,c=document.createElement("div"),c.style.cssText=`
      ...
      font-size: 12px;
      color: #666;
    `,a.addEventListener("change",h),v.appendChild(L),v.appendChild(a),v.appendChild(c);const R=k("label",{text:d.forms.duration,id:"tantrum_duration_label"});R.htmlFor="tantrum_duration",t=k("input",{id:"tantrum_duration"}),t.type="number",t.min="0",t.max="30",t.step="1",t.placeholder="0",t.style.height="40px",v.appendChild(R),v.appendChild(t);const F=k("label",{text:d.forms.coRegulation,id:"tantrum_coreg_label"});F.htmlFor="tantrum_coreg",s=k("div",{id:"tantrum_coreg"}),ue.forEach(M=>{const E=k("input",{id:`tantrum_coreg_${M.value}`});E.type="checkbox",E.value=M.value,E.style.marginRight="8px";const q=k("label",{text:M.label});q.htmlFor=E.id,q.style.marginRight="16px",s.appendChild(E),s.appendChild(q)}),v.appendChild(F),v.appendChild(s);const N=k("label",{text:d.forms.environmentNoise,id:"tantrum_noise_label"});N.htmlFor="tantrum_noise",n=k("input",{id:"tantrum_noise"}),n.type="checkbox",n.style.height="40px",v.appendChild(N),v.appendChild(n);const H=k("label",{text:d.forms.notes,id:"tantrum_notes_label"});H.htmlFor="tantrum_notes",i=k("textarea",{id:"tantrum_notes"}),i.rows=2,i.maxLength=120,i.style.height="60px",o=k("div",{className:"notes-counter"}),ne(i,120,o),v.appendChild(H),v.appendChild(i),v.appendChild(o),f=de(),f.mount(T),p.addEventListener("click",()=>{v.style.display="block",T.style.display="none",p.style.background="#2196F3",p.style.color="white",x.style.background="#f5f5f5",x.style.color="#666"}),x.addEventListener("click",()=>{v.style.display="none",T.style.display="block",p.style.background="#f5f5f5",p.style.color="#666",x.style.background="#2196F3",x.style.color="white",f.refresh()}),r.appendChild(v),r.appendChild(T),g.appendChild(r)}async function h(g){const p=g.target.files?.[0];if(!p){l={},c.textContent="";return}c.textContent=d.placeholders.processing;try{l=await m(p);const x=l.duration_s?`${Math.round(l.duration_s)}s`:"Unknown",v=l.avg_level_dbfs?`${Math.round(l.avg_level_dbfs)}dB`:"Unknown",T=l.motion_estimate?`${Math.round(l.motion_estimate*100)}%`:"Unknown";c.innerHTML=`
        <strong>${p.name}</strong><br>
        Duration: ${x} | Level: ${v}${p.type.startsWith("video")?` | Motion: ${T}`:""}
      `}catch(x){console.error("Error processing media:",x),c.textContent=`Error processing ${p.name}`,l={}}}async function m(g){const b={};try{if(g.type.startsWith("audio/")){const p=new(window.AudioContext||window.webkitAudioContext),x=await g.arrayBuffer(),v=await p.decodeAudioData(x);b.duration_s=v.duration;const T=v.getChannelData(0),B=T.length;let I=0;for(let L=0;L<B;L+=1e3)I+=Math.abs(T[L]);const _=I/(B/1e3);b.avg_level_dbfs=20*Math.log10(_)+60,p.close()}else if(g.type.startsWith("video/")){const p=document.createElement("video");p.src=URL.createObjectURL(g),await new Promise(x=>{p.onloadedmetadata=()=>{b.duration_s=p.duration,b.motion_estimate=Math.random()*.5+.2,URL.revokeObjectURL(p.src),x()}})}}catch(p){console.warn("Media processing failed:",p)}return b}function y(){const g={};e.value&&(g.trigger=e.value);const b=t.value.trim();if(b!==""){const v=Number(b);isNaN(v)||(g.duration_min=se(v,0,30))}const p=ie(s,"input[type=checkbox]");p.length&&(g.co_reg=p),n.checked&&(g.environment_noise=!0);const x=i.value.trim();return x&&(g.notes=x.slice(0,120)),g.intensity_1_10=w.getValue(),Object.keys(l).length>0&&(g.tantrum_proxy=l),g}function S(){const g=t.value.trim();if(g!==""){const b=Number(g);if(isNaN(b)||b<0||b>30)return{ok:!1,message:"Duration must be 0–30"}}return{ok:!0}}async function C(){e.value="",t.value="",s.querySelectorAll("input[type=checkbox]").forEach(g=>g.checked=!1),n.checked=!1,i.value="",o.textContent="0/120",w.setValue(5),a.value="",l={},c.textContent=""}return{mount:u,getContext:y,validate:S,reset:C}}class ge{config;router;container;currentScreen=null;tantrumForm=null;constructor(){this.config=this.parseUrlParams(),console.log("App config:",this.config),this.stripTokenFromUrl(),this.container=document.getElementById("app"),this.router=new j,this.initializeUI(),this.setupRoutes()}parseUrlParams(){const e=new URLSearchParams(window.location.search);return{mode:e.get("mode")||"helper",family:e.get("family")||"fam_unknown",session:e.get("session")||`fam_unknown_${Date.now()}`,token:e.get("tok")||null,dyad:e.get("dyad")||"night"}}initializeUI(){window.location.hash||(this.config.dyad==="tantrum"?window.location.hash="#tantrum/home":this.config.dyad==="meal"?window.location.hash="#meal/home":window.location.hash="#night/home")}setupRoutes(){this.router.register({dyad:"tantrum",screen:"home"},()=>{this.renderScreen(new U(this.container,this.router))}),this.router.register({dyad:"tantrum",screen:"capture"},()=>{const t=this.router.getCurrentRoute()?.params?.intensity||"5";this.renderScreen(new V(this.container,this.router,t))}),this.router.register({dyad:"tantrum",screen:"thermo"},()=>{const e=this.router.getCurrentRoute(),t=e?.params?.intensity||"5",s=e?.params?.hasAudio||"false",n=e?.params?.hasVideo||"false";this.renderScreen(new z(this.container,this.router,t,s,n))}),this.router.register({dyad:"tantrum",screen:"history"},()=>{this.renderScreen(new G(this.container,this.router))}),this.router.register({dyad:"tantrum",screen:"settings"},()=>{this.renderScreen(new W(this.container,this.router))}),this.router.register({dyad:"tantrum",screen:"form"},()=>{this.renderTantrumForm()}),this.router.register({dyad:"meal",screen:"home"},()=>{this.renderScreen(new Y(this.container,this.router))}),this.router.register({dyad:"meal",screen:"meal-logging"},()=>{const e=this.router.getCurrentRoute(),t=e?.params?.action||"",s=e?.params?.rating||"0";this.renderScreen(new J(this.container,this.router,t,s))}),this.router.register({dyad:"meal",screen:"insights"},()=>{const e=this.router.getCurrentRoute(),t=e?.params?.rating||"0",s=e?.params?.hasImage||"false",n=e?.params?.dietaryDiversity||"0.5",i=e?.params?.clutterScore||"0.5",o=e?.params?.plateCoverage||"0.5",a=e?.params?.mode||"patterns";this.renderScreen(new X(this.container,this.router,t,s,n,i,o,a))}),this.router.register({dyad:"meal",screen:"gallery"},()=>{this.renderScreen(new ee(this.container,this.router))}),this.router.register({dyad:"meal",screen:"settings"},()=>{this.renderScreen(new te(this.container,this.router))}),this.router.register({dyad:"night",screen:"home"},()=>{this.renderNightScreen()})}renderTantrumForm(){this.currentScreen&&this.currentScreen.destroy&&this.currentScreen.destroy(),this.container.innerHTML=`
    ...
    `,console.log("Night helper screen rendered")}stripTokenFromUrl(){if(this.config.token){const e=new URL(window.location.href);e.searchParams.delete("tok"),window.history.replaceState({},"",e.toString())}}}document.addEventListener("DOMContentLoaded",()=>{new ge});
