(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(n){if(n.ep)return;n.ep=!0;const a=t(n);fetch(n.href,a)}})();class V{routes=new Map;currentRoute=null;constructor(){this.handleHashChange=this.handleHashChange.bind(this),window.addEventListener("hashchange",this.handleHashChange),window.addEventListener("load",this.handleHashChange)}register(e,t){const s=this.routeToKey(e);this.routes.set(s,t)}navigate(e){const t=this.routeToHash(e);window.location.hash=t}getCurrentRoute(){return this.currentRoute}routeToKey(e){return`${e.dyad}:${e.screen}`}routeToHash(e){let t=`#${e.dyad}/${e.screen}`;if(e.params){const s=new URLSearchParams(e.params);t+=`?${s.toString()}`}return t}hashToRoute(e){const t=e.substring(1),[s,n]=t.split("?"),[a,o]=s.split("/"),i={dyad:a||"night",screen:o||"home"};if(n){const c=new URLSearchParams(n);i.params=Object.fromEntries(c.entries())}return i}handleHashChange(){const e=window.location.hash||"#night/home",t=this.hashToRoute(e);this.currentRoute=t;const s=this.routeToKey(t),n=this.routes.get(s);if(n)n();else{console.warn(`No handler found for route: ${s}`);const a=`${t.dyad}:home`,o=this.routes.get(a);o&&o()}}destroy(){window.removeEventListener("hashchange",this.handleHashChange),window.removeEventListener("load",this.handleHashChange)}}const D={en:{pwa:{title:"Parent Night Helper",mode_label:"Helper mode",button:{start:"Start session",stop:"Stop session",process:"Process results"},score_ring:{label:"Wind-down score",unit:"/100"},badges:{label:"Signals detected"},badge:{speech:"Speech present",music:"Music or TV",fluctuating:"Unsteady sounds",steady:"Steady signal"},tips:{title:"Try this"},tip:{lower_volume:"Lower TV or music volume.",single_sound:"Use one steady sound (e.g. fan or white noise).",soft_light:"Soften the room light.",lower_voice:"Speak slower and softer."},session:{info_id:"Session: {id}",duration:"Duration: {mm}:{ss}"},upload:{success:"✅ Results saved. Check Telegram for details.",failed:"Session saved locally. Will retry. Use /ingest if needed."},permission:{mic_denied:"Microphone access denied. Please allow mic to use this tool.",mic_not_found:"No microphone found. Try restarting or check your device.",error_generic:"Can't access mic. Please check your settings."},footer_privacy:"On-device only. No audio is uploaded. [Privacy Policy]"},meal:{title:"Meal Tracker",subtitle:"Log your baby's feeding moments",sections:{howWasMeal:"How was the meal?",whatWouldYouLike:"What would you like to do?",quickActions:"Quick actions"},buttons:{snapMeal:"Snap a meal photo",askQuestion:"Ask a question",logMeal:"Log a meal",viewPatterns:"View feeding patterns",gallery:"View gallery",settings:"Settings",takePhoto:"Take photo",uploadPhoto:"Upload photo",saveMeal:"Save meal",cancel:"Cancel",back:"Back"},descriptions:{selectRating:"Select a rating for the meal",takePhotoOfMeal:"Take a photo of the meal to remember it better",getInsightsAboutFeeding:"Get insights about feeding patterns"},privacy:{photosStayPrivate:"Photos stay private on your device",dataStaysLocal:"Data stays local on your device"},form:{mealType:"Meal type",foodItems:"Food items",rating:"Rating",notes:"Notes"},insights:{title:"Meal Insights",moodScore:"Mood score",imageAnalysis:"Image analysis",personalizedTip:"Personalized tip",mealAchievement:"Meal achievement"},question:{title:"Ask AI",placeholder:"What's on your mind?",suggestions:"Suggestions",askAI:"Ask AI",aiResponse:"AI response",askAnother:"Ask another"}}},pt_br:{pwa:{title:"Auxiliar da Noite",mode_label:"Modo de ajuda",button:{start:"Iniciar sessão",stop:"Parar sessão",process:"Ver resultados"},score_ring:{label:"Índice de relaxamento",unit:"/100"},badges:{label:"Sinais detectados"},badge:{speech:"Voz ativa",music:"Música ou TV",fluctuating:"Sons instáveis",steady:"Som contínuo"},tips:{title:"Tente isto"},tip:{lower_volume:"Abaixe o volume da TV ou música.",single_sound:"Use um som contínuo (como ventilador ou ruído branco).",soft_light:"Diminua a luz do ambiente.",lower_voice:"Fale mais devagar e com suavidade."},session:{info_id:"Sessão: {id}",duration:"Duração: {mm}:{ss}"},upload:{success:"✅ Resultados salvos. Veja os detalhes no Telegram.",failed:"Sessão salva localmente. Vamos tentar de novo. Use /ingest se precisar."},permission:{mic_denied:"Acesso ao microfone negado. Permita o uso para continuar.",mic_not_found:"Nenhum microfone encontrado. Reinicie ou verifique o dispositivo.",error_generic:"Não foi possível acessar o microfone. Verifique as configurações."},footer_privacy:"Análise local. Nenhum áudio é enviado. [Política de Privacidade]"},meal:{title:"Registro de Refeição",subtitle:"Registre os momentos de alimentação do seu bebê",sections:{howWasMeal:"Como foi a refeição?",whatWouldYouLike:"O que você gostaria de fazer?",quickActions:"Ações rápidas"},buttons:{snapMeal:"Fotografe uma foto da refeição",askQuestion:"Faça uma pergunta",logMeal:"Registre uma refeição",viewPatterns:"Veja padrões de alimentação",gallery:"Veja galeria",settings:"Configurações",takePhoto:"Tirar foto",uploadPhoto:"Enviar foto",saveMeal:"Salvar refeição",cancel:"Cancelar",back:"Voltar"},descriptions:{selectRating:"Selecione uma avaliação para a refeição",takePhotoOfMeal:"Fotografe uma foto da refeição para lembrar melhor",getInsightsAboutFeeding:"Obtenha insights sobre padrões de alimentação"},privacy:{photosStayPrivate:"Fotos ficam privadas no seu dispositivo",dataStaysLocal:"Dados ficam locais no seu dispositivo"},form:{mealType:"Tipo de refeição",foodItems:"Itens alimentares",rating:"Avaliação",notes:"Notas"},insights:{title:"Insights de Refeição",moodScore:"Pontuação de humor",imageAnalysis:"Análise de imagem",personalizedTip:"Dica personalizada",mealAchievement:"Conquista de refeição"},question:{title:"Perguntar ao AI",placeholder:"O que está na sua cabeça?",suggestions:"Sugestões",askAI:"Perguntar ao AI",aiResponse:"Resposta do AI",askAnother:"Perguntar outra"}}}};class z{currentLanguage="en";constructor(){this.detectLanguage()}detectLanguage(){const t=new URLSearchParams(window.location.search).get("lang");if(t&&D[t]){this.currentLanguage=t;return}navigator.language.toLowerCase().startsWith("pt")?this.currentLanguage="pt_br":this.currentLanguage="en";const n=localStorage.getItem("silli_language");n&&D[n]&&(this.currentLanguage=n)}setLanguage(e){if(D[e]){this.currentLanguage=e,localStorage.setItem("silli_language",e);const t=new URL(window.location.href);t.searchParams.set("lang",e),window.history.replaceState({},"",t.toString())}}getLanguage(){return this.currentLanguage}t(e,t){const s=e.split(".");let n=D[this.currentLanguage];for(const a of s)if(n&&typeof n=="object"&&a in n)n=n[a];else{n=this.getFallbackValue(e);break}return typeof n!="string"?this.getFallbackValue(e):t?n.replace(/\{(\w+)\}/g,(a,o)=>t[o]?.toString()||a):n}getFallbackValue(e){const t=e.split(".");let s=D.en;for(const n of t)if(s&&typeof s=="object"&&n in s)s=s[n];else return e;return typeof s=="string"?s:e}getAvailableLanguages(){return Object.keys(D)}getLanguageName(e){return{en:"English",pt_br:"Português (Brasil)"}[e]||e}}const T=new z;class W{container;isOpen=!1;constructor(e){this.container=e}render(){const e=T.getLanguage(),t=T.getAvailableLanguages();this.container.innerHTML=`
...
If you have specific concerns about your child's eating, consider consulting with a pediatrician or registered dietitian.`}displayAnswer(e){const t=this.container.querySelector("#answer-section"),s=this.container.querySelector("#answer-content");if(t&&s){t.style.display="block",s.innerHTML=`
    ...
    `,r.appendChild(h);const u=document.createElement("div");u.className="history-stats",u.style.cssText=`
      ...
      border-radius: 6px;
      font-size: 14px;
    `,r.appendChild(u);const v=document.createElement("div");v.className="history-sessions",v.style.cssText=`
      display: flex;
      flex-direction: column;
      gap: 8px;
    `,r.appendChild(v),l.appendChild(r),s()}async function s(){try{e=await F.getSessions(),n(),o()}catch(l){console.error("Error loading history:",l),i()}}function n(){const l=r.querySelector(".history-stats");if(!l)return;const h={total:e.length,avg_intensity:e.length>0?Math.round(e.reduce((u,v)=>u+(v.intensity_1_10||0),0)/e.length*10)/10:0,most_common_trigger:a()};l.innerHTML=`
    ...
    `}function a(){const l=e.map(u=>u.trigger).filter(u=>u!==void 0);if(l.length===0)return null;const h=l.reduce((u,v)=>(u[v]=(u[v]||0)+1,u),{});return Object.entries(h).reduce((u,v)=>u[1]>v[1]?u:v)[0]}function o(){const l=r.querySelector(".history-sessions");if(l){if(e.length===0){l.innerHTML=`
        <div style="text-align: center; padding: 32px; color: #666; font-style: italic;">
          ${d.empty.noSessionsYet}
        </div>
      `;return}l.innerHTML=e.map(h=>{const u=new Date(h.ts),v=w(u),y=h.intensity_1_10||0,x=h.escalation_index||0,L=y<=3?"#4CAF50":y<=6?"#FF9800":"#F44336",g=x<=.3?"#4CAF50":x<=.7?"#FF9800":"#F44336";return`
    ...
    `)}function c(l){const u=Math.abs(new Date().getTime()-l.getTime()),v=Math.ceil(u/(1e3*60*60*24));return v===1?"Today":v===2?"Yesterday":v<=7?l.toLocaleDateString("en-US",{weekday:"short"}):l.toLocaleDateString("en-US",{month:"short",day:"numeric"})}function w(l){const u=new Date().getTime()-l.getTime(),v=Math.floor(u/(1e3*60)),y=Math.floor(u/(1e3*60*60)),x=Math.floor(u/(1e3*60*60*24));return v<1?"Just now":v<60?`${v}m ago`:y<24?`${y}h ago`:x<7?`${x}d ago`:l.toLocaleDateString("en-US",{month:"short",day:"numeric"})}function f(){r&&r.parentNode&&r.parentNode.removeChild(r)}return{mount:t,refresh:s,destroy:f}}const me=d.triggers,ve=d.coRegulation;function ye(){let r,e,t,s,n,a,o,i,c,w,f,l={};function h(g){g.innerHTML="",r=k("div",{className:"form form--tantrum"});const b=document.createElement("div");b.style.cssText=`
      display: flex;
      margin-bottom: 16px;
      border-bottom: 2px solid #e0e0e0;
    `;const p=document.createElement("button");p.textContent=d.tabs.recordSession,p.style.cssText=`
      ...
      font-weight: 600;
      cursor: pointer;
    `;const S=document.createElement("button");S.textContent=d.tabs.history,S.style.cssText=`
      flex: 1;
      padding: 12px;
      border: none;
      background: #f5f5f5;
      ...
      font-weight: 600;
      cursor: pointer;
    `,b.appendChild(p),b.appendChild(S),r.appendChild(b);const m=document.createElement("div");m.className="form-content",m.style.cssText=`
      display: block;
    `;const C=document.createElement("div");C.className="history-content",C.style.cssText=`
      display: none;
    `;const P=k("label",{text:d.forms.trigger,id:"tantrum_trigger_label"});P.htmlFor="tantrum_trigger",e=k("select",{id:"tantrum_trigger"});const A=document.createElement("option");A.text=d.placeholders.selectTrigger,A.value="",e.appendChild(A),me.forEach(M=>{const E=document.createElement("option");E.text=M.charAt(0).toUpperCase()+M.slice(1),E.value=M,e.appendChild(E)}),m.appendChild(P),m.appendChild(e);const B=document.createElement("div");B.style.cssText=`
      margin: 16px 0;
      padding: 16px;
      background: #f9f9f9;
      border-radius: 8px;
    `,w=de(),w.mount(B),m.appendChild(B);const I=k("label",{text:"Media Clip (Optional)",id:"tantrum_media_label"});I.htmlFor="tantrum_media",i=k("input",{id:"tantrum_media"}),i.type="file",i.accept="audio/*,video/*",i.capture="environment",i.style.cssText=`
      margin: 8px 0;
      padding: 8px;
    ...
    `,c=document.createElement("div"),c.style.cssText=`
      ...
      font-size: 12px;
      color: #666;
    `,i.addEventListener("change",u),m.appendChild(I),m.appendChild(i),m.appendChild(c);const N=k("label",{text:d.forms.duration,id:"tantrum_duration_label"});N.htmlFor="tantrum_duration",t=k("input",{id:"tantrum_duration"}),t.type="number",t.min="0",t.max="30",t.step="1",t.placeholder="0",t.style.height="40px",m.appendChild(N),m.appendChild(t);const O=k("label",{text:d.forms.coRegulation,id:"tantrum_coreg_label"});O.htmlFor="tantrum_coreg",s=k("div",{id:"tantrum_coreg"}),ve.forEach(M=>{const E=k("input",{id:`tantrum_coreg_${M.value}`});E.type="checkbox",E.value=M.value,E.style.marginRight="8px";const R=k("label",{text:M.label});R.htmlFor=E.id,R.style.marginRight="16px",s.appendChild(E),s.appendChild(R)}),m.appendChild(O),m.appendChild(s);const H=k("label",{text:d.forms.environmentNoise,id:"tantrum_noise_label"});H.htmlFor="tantrum_noise",n=k("input",{id:"tantrum_noise"}),n.type="checkbox",n.style.height="40px",m.appendChild(H),m.appendChild(n);const j=k("label",{text:d.forms.notes,id:"tantrum_notes_label"});j.htmlFor="tantrum_notes",a=k("textarea",{id:"tantrum_notes"}),a.rows=2,a.maxLength=120,a.style.height="60px",o=k("div",{className:"notes-counter"}),re(a,120,o),m.appendChild(j),m.appendChild(a),m.appendChild(o),f=ge(),f.mount(C),p.addEventListener("click",()=>{m.style.display="block",C.style.display="none",p.style.background="#2196F3",p.style.color="white",S.style.background="#f5f5f5",S.style.color="#666"}),S.addEventListener("click",()=>{m.style.display="none",C.style.display="block",p.style.background="#f5f5f5",p.style.color="#666",S.style.background="#2196F3",S.style.color="white",f.refresh()}),r.appendChild(m),r.appendChild(C),g.appendChild(r)}async function u(g){const p=g.target.files?.[0];if(!p){l={},c.textContent="";return}c.textContent=d.placeholders.processing;try{l=await v(p);const S=l.duration_s?`${Math.round(l.duration_s)}s`:"Unknown",m=l.avg_level_dbfs?`${Math.round(l.avg_level_dbfs)}dB`:"Unknown",C=l.motion_estimate?`${Math.round(l.motion_estimate*100)}%`:"Unknown";c.innerHTML=`
        <strong>${p.name}</strong><br>
        Duration: ${S} | Level: ${m}${p.type.startsWith("video")?` | Motion: ${C}`:""}
      `}catch(S){console.error("Error processing media:",S),c.textContent=`Error processing ${p.name}`,l={}}}async function v(g){const b={};try{if(g.type.startsWith("audio/")){const p=new(window.AudioContext||window.webkitAudioContext),S=await g.arrayBuffer(),m=await p.decodeAudioData(S);b.duration_s=m.duration;const C=m.getChannelData(0),P=C.length;let A=0;for(let I=0;I<P;I+=1e3)A+=Math.abs(C[I]);const B=A/(P/1e3);b.avg_level_dbfs=20*Math.log10(B)+60,p.close()}else if(g.type.startsWith("video/")){const p=document.createElement("video");p.src=URL.createObjectURL(g),await new Promise(S=>{p.onloadedmetadata=()=>{b.duration_s=p.duration,b.motion_estimate=Math.random()*.5+.2,URL.revokeObjectURL(p.src),S()}})}}catch(p){console.warn("Media processing failed:",p)}return b}function y(){const g={};e.value&&(g.trigger=e.value);const b=t.value.trim();if(b!==""){const m=Number(b);isNaN(m)||(g.duration_min=oe(m,0,30))}const p=ce(s,"input[type=checkbox]");p.length&&(g.co_reg=p),n.checked&&(g.environment_noise=!0);const S=a.value.trim();return S&&(g.notes=S.slice(0,120)),g.intensity_1_10=w.getValue(),Object.keys(l).length>0&&(g.tantrum_proxy=l),g}function x(){const g=t.value.trim();if(g!==""){const b=Number(g);if(isNaN(b)||b<0||b>30)return{ok:!1,message:"Duration must be 0–30"}}return{ok:!0}}async function L(){e.value="",t.value="",s.querySelectorAll("input[type=checkbox]").forEach(g=>g.checked=!1),n.checked=!1,a.value="",o.textContent="0/120",w.setValue(5),i.value="",l={},c.textContent=""}return{mount:h,getContext:y,validate:x,reset:L}}class fe{config;router;container;currentScreen=null;tantrumForm=null;constructor(){this.config=this.parseUrlParams(),console.log("App config:",this.config),this.stripTokenFromUrl(),this.container=document.getElementById("app"),this.router=new V,this.initializeUI(),this.setupRoutes()}parseUrlParams(){const e=new URLSearchParams(window.location.search);return{mode:e.get("mode")||"helper",family:e.get("family")||"fam_unknown",session:e.get("session")||`fam_unknown_${Date.now()}`,token:e.get("tok")||null,dyad:e.get("dyad")||"night"}}initializeUI(){window.location.hash||(this.config.dyad==="tantrum"?window.location.hash="#tantrum/home":this.config.dyad==="meal"?window.location.hash="#meal/home":window.location.hash="#night/home")}setupRoutes(){this.router.register({dyad:"tantrum",screen:"home"},()=>{this.renderScreen(new G(this.container,this.router))}),this.router.register({dyad:"tantrum",screen:"capture"},()=>{const t=this.router.getCurrentRoute()?.params?.intensity||"5";this.renderScreen(new Y(this.container,this.router,t))}),this.router.register({dyad:"tantrum",screen:"thermo"},()=>{const e=this.router.getCurrentRoute(),t=e?.params?.intensity||"5",s=e?.params?.hasAudio||"false",n=e?.params?.hasVideo||"false";this.renderScreen(new K(this.container,this.router,t,s,n))}),this.router.register({dyad:"tantrum",screen:"history"},()=>{this.renderScreen(new J(this.container,this.router))}),this.router.register({dyad:"tantrum",screen:"settings"},()=>{this.renderScreen(new Q(this.container,this.router))}),this.router.register({dyad:"tantrum",screen:"form"},()=>{this.renderTantrumForm()}),this.router.register({dyad:"meal",screen:"home"},()=>{this.renderScreen(new Z(this.container,this.router))}),this.router.register({dyad:"meal",screen:"meal-logging"},()=>{const e=this.router.getCurrentRoute(),t=e?.params?.action||"",s=e?.params?.rating||"0";this.renderScreen(new X(this.container,this.router,t,s))}),this.router.register({dyad:"meal",screen:"insights"},()=>{const e=this.router.getCurrentRoute(),t=e?.params?.rating||"0",s=e?.params?.hasImage||"false",n=e?.params?.dietaryDiversity||"0.5",a=e?.params?.clutterScore||"0.5",o=e?.params?.plateCoverage||"0.5",i=e?.params?.mode||"patterns";this.renderScreen(new ne(this.container,this.router,t,s,n,a,o,i))}),this.router.register({dyad:"meal",screen:"gallery"},()=>{this.renderScreen(new ae(this.container,this.router))}),this.router.register({dyad:"meal",screen:"settings"},()=>{this.renderScreen(new ie(this.container,this.router))}),this.router.register({dyad:"night",screen:"home"},()=>{this.renderNightScreen()})}renderTantrumForm(){this.currentScreen&&this.currentScreen.destroy&&this.currentScreen.destroy(),this.container.innerHTML=`
